...
exports.create = async orderData => {
	try {
		...
		for (let n = 0; n < orderData.wantedProducts.length; n++)
			await Verification.IdExists(
				Resources.Products,
				orderData.wantedProducts[n].id
			)

		const order = await orderRepo.createOrder(orderData)

		await this.addProducts(order, orderData.wantedProducts)
...
exports.addProducts = async (order, productsArray) => {
	try {
		for (let n = 0; n < productsArray.length; n++) {
			const productInOrder = await order.getProducts({
				where: { id: productsArray[n].id }
			})

			if (productInOrder.length !== 0) continue
			const product = await productRepo.getById(productsArray[n].id)

			await OrderHelpers.addQuantityAndTotalPrice(
				order,
				product,
				productsArray[n].quantity
			)
			await orderRepo.updateOrder(order.id, order.dataValues)
			await orderRepo.addProducts(
				order,
				productsArray[n].id,
				productsArray[n].quantity
			)
...
exports.deleteProducts = async (orderId, productsArray) => {
	try {
		...
		for (let n = 0; n < productsArray.length; n++) {
			...
			await OrderHelpers.subtractQuantityAndTotalPrice(
				order,
				productToDelete,
				quantity
			)
...
exports.changeCurrency = async (prevOrder, updatedOrder) => {
	...
	await this.deleteProductsOldCurrency(
		prevOrder,
		updatedOrder,
		productsToUpdate
	)
...
exports.deleteProductsOldCurrency = async (
	prevOrder,
	updatedOrder,
	productsToUpdate
) => {
	for (let n = 0; n < productsToUpdate.length; n++) {
		const productInOrder = await prevOrder.getProducts({
			where: { id: productsToUpdate[n].id }
		})
		...
		updatedOrder.dataValues.productsTotalPrice =
			prevOrder.dataValues.productsTotalPrice
...
exports.updateProducts = async (order, productsArray) => {
	for (let n = 0; n < productsArray.length; n++) {
		...
		if (productInOrder.length === 0) {
			...
			const prevQuantity = productInOrder[0].products_orders.quantity
			await this.updateOrderQuantityUpdate(
				order,
				product,
				prevQuantity,
				newQuantity
...
exports.updateOrderQuantityUpdate = async (
	order,
	product,
	prevQuantity,
	newQuantity
