...
exports.update = async (request, response, next) => {
	try {
		...
		const affectedRows = await customersDomain.update(
			customerId,
			updateCustomerData
		)
...
exports.getSearchResults = async (request, response) => {
	try {
		const filteredCustomers = await customersDomain.getSearchResults(
			request.query.search
		)

		response.status(200).json(filteredCustomers)
	} catch (erorr) {
		response.status(error.code).json(error)
	}
}
